...
export interface CustomData {
  /** The index in the original spine. Normally only present if the file was generated by {@link getInputContext} */
  spineIndex?: number;
  [key: string]: any;
}
...
export async function getInputContext(inputPath: string): Promise<EpubContext<BaseEpubOptions, InputEpubCustomData>> {
  ...
  for (const item of Array.from(xh.queryDefinedElementAll(manifestElem, 'item'))) {
    ...
    const spineIndex = spineChildren.findIndex((v) => v.nodeType === v.ELEMENT_NODE && (v as Element).getAttribute('idref') === found.id);

    // dont add index as data if not in the spine
    if (spineIndex >= 0) {
